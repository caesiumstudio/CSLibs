...
public class IAPAndroidStore : IAPStore
{
    private Logger logger = new Logger("IAPAndroidStore");
    ...
    public void SetOnProductsLoaded(OnProductsLoaded fun)
    {
        ...
        logger.Log("added callback: " + this.OnProductsLoadedCB);
        logger.Log("added callback fun: " + fun);
    }
    ...
    private static IAPAndroidStore instance = new IAPAndroidStore();

    public static IAPAndroidStore getInstance()
    {
        if (IAPAndroidStore.instance == null)
            IAPAndroidStore.instance = new IAPAndroidStore();

        return IAPAndroidStore.instance;
    }

    public async void LoadIAPProductCatalog()
    {
        logger.Log("Android Loading IPA Catalog");
    ...
    public void OnInitialized(IStoreController controller, IExtensionProvider extensions)
    {
        ...
        for (int i = 0; i < storeController.products.all.Length; i++)
        {
            ...
            {
                logger.Log("AVAILABLE TO PURCHASE - " + product.definition.id + ": " + product.metadata.localizedPriceString);
        ...
        this.OnProductsLoadedCB(availableProducts);
    }
